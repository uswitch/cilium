    ...
    stages {
        ...
        stage ("Copy code and boot vms"){
            ...
            parallel {
                ...
                stage('Boot vms K8s-1.17 net-next kubeproxy-free') {
                    environment {
                        TESTED_SUITE="k8s-1.17-kubeproxy-free"
                        GOPATH="${WORKSPACE}/${TESTED_SUITE}-gopath"
                        TESTDIR="${GOPATH}/${PROJ_PATH}/test"
                        NETNEXT="true"
                        K8S_VERSION="1.17"
        ...
        stage ("BDD-Test-PR"){
            ...
            parallel {
                ...
                stage('BDD-Test-PR-K8s-1.17-net-next-kubeproxy-free') {
                    environment {
                        TESTED_SUITE="k8s-1.17"
                        GOPATH="${WORKSPACE}/${TESTED_SUITE}-gopath"
                        TESTDIR="${GOPATH}/${PROJ_PATH}/test"
                    ...
                    steps {
                        sh 'cd ${TESTDIR}; ginkgo --focus="$(echo ${ghprbCommentBody} | sed -r "s/([^ ]* |^[^ ]*$)//" | sed "s/^$/K8s*/" | sed "s/Runtime.*/NoTests/")" --focus ".*Tests NodePort BPF.*" -v --failFast=${FAILFAST} -- -cilium.provision=false -cilium.timeout=${GINKGO_TIMEOUT} -cilium.kubeconfig=${TESTDIR}/vagrant-kubeconfig -cilium.passCLIEnvironment=true -cilium.registry=$(print-node-ip.sh)'
                    }
                    post {
                        always {
                            sh 'cd ${TESTDIR}; ./post_build_agent.sh || true'
